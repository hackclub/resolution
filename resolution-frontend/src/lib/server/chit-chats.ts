...
export async function createChitChatsShipment(params: {
	...
	const shipmentBody: Record<string, any> = {
		...
		city: order.city,
		...((order.country === 'CA' || order.country === 'US') && order.stateProvince ? { province_code: order.stateProvince } : {}),
...
export async function fetchChitChatsRates(params: {
	...
	const shipmentBody: Record<string, any> = {
		...
		city: params.city,
		...((params.country === 'CA' || params.country === 'US') && params.province ? { province_code: params.province } : {}),
